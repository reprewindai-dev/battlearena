...
async function completeBlueprintReset() {
  ...
  try {
    ...
    console.log('📋 All existing tables:', existingTables.map(t => t.table_name));

    // Drop ALL public tables for clean slate
    const allTableNames = existingTables.map(t => t.table_name);
    if (allTableNames.length > 0) {
      console.log('🗑️ Dropping ALL existing tables...');
      await client.query(`DROP TABLE IF EXISTS ${allTableNames.join(', ')} CASCADE`);
      console.log('✅ All tables dropped');
    }

    // Implement the complete blueprint schema from scratch
    const schema = `
...
`;
    ...
    console.log('✅ Complete blueprint schema created');
...
CREATE INDEX idx_battles_participants ON public.battles(participant_1_id, participant_2_id);
CREATE INDEX idx_battles_tournament ON public.battles(tournament_id);
CREATE INDEX idx_user_ratings_tier ON public.user_ratings(tier);
...
CREATE INDEX idx_ai_flags_status ON public.ai_moderation_flags(status);
...
CREATE INDEX idx_lobbies_type ON public.lobbies(lobby_type);
CREATE INDEX idx_lobbies_active ON public.lobbies(is_active);
CREATE INDEX idx_lobbies_featured ON public.lobbies(is_featured);
CREATE INDEX idx_matchmaking_queue_user ON public.matchmaking_queue(user_id);
CREATE INDEX idx_matchmaking_queue_type ON public.matchmaking_queue(queue_type);
`;
    ...
    await insertComprehensiveSampleData(client);

    await client.end();
    console.log('🔌 Disconnected cleanly');
    
    console.log('\n🎉 Complete blueprint schema implementation finished!');
    console.log('📊 Database now matches the original blueprint exactly');
    console.log('🏗️ All systems: Users, Battles, Economy, Tournaments, Moderation, Community, Rooms');
...
async function insertComprehensiveSampleData(client) {
  ...
  await client.query(`
    ...
    VALUES ($1, 'Test User', 'Testing the complete battle arena system')
  ...
  await client.query(`
    INSERT INTO public.tournaments (name, description, tournament_type, format, status, max_participants, entry_fee_tokens, prize_pool_tokens, created_by)
    VALUES ('Weekly Battle Royale', 'Weekly tournament with cash prizes', 'weekly', 'single_elimination', 'upcoming', 64, 50, 1000, $1)
    ON CONFLICT DO NOTHING
  `, [testUserId]);

  // Insert sample crew
  await client.query(`
    INSERT INTO public.crews (name, tag, description, leader_id)
    VALUES ('Elite MCs', 'ELITE', 'Top tier battle rap crew', $1)
