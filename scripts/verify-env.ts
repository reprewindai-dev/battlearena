...
const REQUIRED_ENV_VARS: EnvVar[] = [
  ...
  { name: 'SUPABASE_SERVICE_ROLE_KEY', required: true, pattern: /^eyJ/, description: 'Supabase service role key' },
  { name: 'NEXT_PUBLIC_SUPABASE_ANON_KEY', required: true, pattern: /^eyJ/, description: 'Supabase anonymous key' },
  
  // LiveKit
  { name: 'LIVEKIT_HOST', required: true, description: 'LiveKit server host' },
  { name: 'LIVEKIT_API_KEY', required: true, pattern: /^[A-Za-z0-9]+$/, description: 'LiveKit API key' },
  { name: 'LIVEKIT_API_SECRET', required: true, pattern: /^[A-Za-z0-9]+$/, description: 'LiveKit API secret' },
  ...
  { name: 'JWT_REFRESH_SECRET', required: true, pattern: /^.{32,}$/, description: 'JWT refresh secret (min 32 chars)' },
  
  // Stripe
  { name: 'STRIPE_PUBLISHABLE_KEY', required: true, pattern: /^pk_/, description: 'Stripe publishable key' },
  { name: 'STRIPE_SECRET_KEY', required: true, pattern: /^sk_/, description: 'Stripe secret key' },
  { name: 'STRIPE_WEBHOOK_SECRET', required: true, pattern: /^whsec_/, description: 'Stripe webhook secret' },
  
  // AWS S3
  { name: 'AWS_ACCESS_KEY_ID', required: true, pattern: /^[A-Z0-9]+$/, description: 'AWS access key' },
  { name: 'AWS_SECRET_ACCESS_KEY', required: true, pattern: /^[A-Za-z0-9\/]+$/, description: 'AWS secret key' },
...
for (const envVar of REQUIRED_ENV_VARS) {
  ...
  if (!value) {
    ...
    if (envVar.pattern && !envVar.pattern.test(value)) {
      errors.push(`❌ INVALID FORMAT: ${envVar.name} (${value})`);
      failed++;
    } else {
    ...
    if (envVar.name.toLowerCase().includes('secret') || 
