...
export class TelemetrySystem {
  ...
  async emitEvent(event: TelemetryEvent): Promise<void> {
    ...
    if (this.eventBuffer.length >= this.BUFFER_SIZE || Date.now() - this.lastFlushTime > this.FLUSH_INTERVAL) {
  ...
  async calculateRematchRate(timeWindow: number = 3600000): Promise<number> {
    try {
      ...
      const [offersResult, acceptsResult] = await Promise.all([
        this.supabase.from("telemetry_events").select("player_id").eq("event_type", "REMATCH_OFFER_SHOWN").gte("timestamp", startTime.toISOString()),
        this.supabase.from("telemetry_events").select("player_id").eq("event_type", "REMATCH_ACCEPTED").gte("timestamp", startTime.toISOString()),
      ]);
  ...
  async calculateRageQuitRate(timeWindow: number = 3600000): Promise<number> {
    try {
      ...
      const [matchesResult, rageQuitsResult] = await Promise.all([
        this.supabase.from("telemetry_events").select("match_id").eq("event_type", "MATCH_END").gte("timestamp", startTime.toISOString()),
        this.supabase.from("telemetry_events").select("match_id").eq("event_type", "RAGE_QUIT").gte("timestamp", startTime.toISOString()),
      ]);
  ...
  async getMetricsDashboard(timeWindow: number = 3600000): Promise<TelemetryMetrics> {
    ...
    if (this.metricsCache.has(cacheKey)) {
      ...
      if (typeof cached.timestamp === "number" && Date.now() - cached.timestamp < 30000) {
        return cached;
      }
    }
  ...
  private async calculateDisconnectRate(timeWindow: number): Promise<number> {
    ...
    const [matchesResult, disconnectsResult] = await Promise.all([
      this.supabase.from("telemetry_events").select("match_id").eq("event_type", "MATCH_START").gte("timestamp", startTime.toISOString()),
      this.supabase.from("telemetry_events").select("match_id").eq("event_type", "PLAYER_DISCONNECT").gte("timestamp", startTime.toISOString()),
    ]);
  ...
  private async calculateGovernanceBlockRate(timeWindow: number): Promise<number> {
    ...
    const [totalPlansResult, blocksResult] = await Promise.all([
      this.supabase.from("match_opponent_plans").select("id").gte("created_at", startTime.toISOString()),
      this.supabase.from("telemetry_events").select("id").eq("event_type", "GOVERNANCE_BLOCK").gte("timestamp", startTime.toISOString()),
    ]);
  ...
  private async calculateCircuitBreakerRate(timeWindow: number): Promise<number> {
    ...
    return (data?.length || 0) / (timeWindow / 3600000);
  }
  ...
  private async handleRageQuitAlert(event: TelemetryEvent): Promise<void> {
    const recentRageQuits = await this.getRecentEventCount("RAGE_QUIT", 300000);
    const recentMatches = await this.getRecentEventCount("MATCH_END", 300000);

    if (recentMatches > 0 && recentRageQuits / recentMatches > 0.1) {
      await deliverOpsAlert(this.supabase, {
        code: "rage_quit_spike",
        severity: "warning",
        title: "Rage quit spike detected",
        body: `Rage quit rate reached ${(recentRageQuits / recentMatches * 100).toFixed(1)}% across the last ${recentMatches} completed matches.`,
        payload: {
          rage_quits: recentRageQuits,
          matches: recentMatches,
          rate: recentRageQuits / recentMatches,
          trigger: event.event_data,
        },
      });
  ...
  private async handleGovernanceBlockAlert(event: TelemetryEvent): Promise<void> {
    ...
    if (recentBlocks > 5) {
      await deliverOpsAlert(this.supabase, {
        code: "governance_block_spike",
        severity: "warning",
        title: "Governance block spike detected",
        body: `Governance blocked ${recentBlocks} matchmaking plans in the last 5 minutes.`,
        payload: {
          blocks: recentBlocks,
  ...
  private async handleCircuitBreakerAlert(event: TelemetryEvent): Promise<void> {
    ...
    await deliverOpsAlert(this.supabase, {
      code: `circuit_breaker_${String(circuitData.circuit_name ?? "global")}`,
      severity: "critical",
      title: "Circuit breaker opened",
      body: `Circuit ${String(circuitData.circuit_name ?? "global")} opened after repeated failures.`,
      payload: circuitData as Record<string, unknown>,
  ...
  private async handleFairnessViolationAlert(event: TelemetryEvent): Promise<void> {
    ...
    await deliverOpsAlert(this.supabase, {
      code: `fairness_violation_${String(violationData.violation_type ?? "unknown")}`,
      severity: "critical",
      title: "Fairness violation detected",
      body: `Violation ${String(violationData.violation_type ?? "unknown")} crossed its configured threshold.`,
      payload: violationData as Record<string, unknown>,
      throttleMs: 5 * 60 * 1000,
    });
  }
