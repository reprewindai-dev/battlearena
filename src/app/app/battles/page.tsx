      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Queue into Battle Arena matchmaking or jump straight into a live room.
          </p>
        </div>
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <div className="mt-1 text-xs text-muted-foreground">
            Your latest battles created from this account.
          </div>
        </div>
          ...
          battles.map((battle) => (
            ...
            >
              <div className="min-w-[240px]">
                <div className="font-mono text-sm text-foreground">{battle.id}</div>
                <div className="mt-1 text-xs text-muted-foreground">
                  {battle.mode} - {battle.status}
                  {battle.created_at ? ` - ${new Date(battle.created_at).toLocaleString()}` : ""}
                </div>
              </div>
