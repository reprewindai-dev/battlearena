...
export default function OAuthButtons({ mode, nextPath = "/app" }: OAuthButtonsProps) {
  ...
  async function signInWithProvider(provider: "google" | "apple") {
    ...
    try {
      ...
      // If successful, browser redirects to provider
    } catch (err) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        disabled={!!loading}
        className="flex w-full items-center justify-center gap-3 rounded-lg border border-white/10 bg-white px-4 py-3 text-sm font-medium text-gray-900 transition-all hover:bg-gray-50 disabled:opacity-50"
      >
        {loading === "google" ? (
