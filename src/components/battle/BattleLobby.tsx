  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        <AnimatePresence>
          {loading ? (
            // Real loading state with actual content
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="space-y-2">
                        ...
                        <div className="h-3 bg-white/5 rounded animate-pulse w-24"></div>
                      </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="h-4 bg-white/5 rounded animate-pulse w-16"></div>
                    </div>
                    <div className="flex justify-between items-center">
                      <div className="h-3 bg-white/5 rounded animate-pulse w-24"></div>
...
export function BattleLobby({ onJoinBattle, onCreateBattle }: BattleLobbyProps) {
  ...
  useEffect(() => {
    fetchBattles();
    const interval = setInterval(fetchBattles, 5000); // Refresh every 5 seconds
  ...
  const fetchBattles = async () => {
    try {
      ...
      if (filters.battle_type !== 'all') params.append('battle_type', filters.battle_type);
  ...
  const handleJoinBattle = async (battleId: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error?.message || 'Failed to join battle');
      }
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to join battle');
    }
  };

  const filteredBattles = battles.filter(battle => {
    const matchesSearch = battle.created_by.username.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         battle.beat?.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         battle.room_code.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-300">Join or create battles to showcase your skills</p>
          </div>
          
          <Button onClick={onCreateBattle} size="lg" className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700">
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          <span className="font-mono font-bold">{battle.room_code}</span>
                        </div>
                        
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-400">Format</span>
                          <span className="font-semibold">{battle.format}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span>{battle.current_participants}/{battle.max_participants}</span>
                          </div>
                        </div>
                        ...
                        {battle.entry_fee_tokens > 0 && (
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-1">
                              ...
                              <span>{battle.entry_fee_tokens} tokens</span>
                            </div>
                          </div>
                        )}

                        {battle.prize_pool_tokens > 0 && (
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-gray-400">Prize Pool</span>
                            <div className="flex items-center space-x-1">
                              <Trophy className="w-4 h-4 text-yellow-500" />
                              <span>{battle.prize_pool_tokens} tokens</span>
                            </div>
                          </div>
                        )}
                        ...
                        {battle.beat && (
                          <div className="p-3 rounded-lg bg-white/5">
                            <p className="text-sm text-gray-400 mb-1">Beat</p>
                            <p className="font-semibold">{battle.beat.title}</p>
                            <p className="text-xs text-gray-400">{battle.beat.artist} • {battle.beat.tempo} BPM</p>
                          </div>
                        )}
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            <span className="font-semibold">{battle.created_by.username}</span>
                            <Badge variant="outline" className="text-xs">
                              {battle.created_by.tier}
                            </Badge>
                          </div>
                        </div>
                      </div>
                      ...
                      <Button
                        ...
                        disabled={battle.status !== 'waiting' || battle.current_participants >= battle.max_participants}
                        className="w-full"
                        variant={battle.status === 'waiting' ? 'default' : 'secondary'}
                      >
                        {battle.status === 'waiting' ? (
                          battle.current_participants >= battle.max_participants ? 'Full' : 'Join Battle'
                        ) : battle.status === 'active' ? 'In Progress' : 'Completed'}
                      </Button>
                    </CardContent>
                  </Card>
                </motion.div>
