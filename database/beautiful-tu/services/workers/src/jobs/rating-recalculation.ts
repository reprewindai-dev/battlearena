...
const log = createJobLogger('rating-recalculation');

interface GlickoParams {
  rating: number;
  deviation: number;
  volatility: number;
}
...
export const recalculateRatings = async () => {
  ...
  const { data: completedBattles, error } = await supabase
    .from('battles')
  ...
  for (const battle of completedBattles || []) {
    if (!battle.player1_id || !battle.player2_id || !battle.winner_id) continue;

    const [{ data: p1Rating }, { data: p2Rating }] = await Promise.all([
      supabase.from('user_ratings').select('*').eq('user_id', battle.player1_id).single(),
      supabase.from('user_ratings').select('*').eq('user_id', battle.player2_id).single(),
    ]);
    ...
    const newP1 = calculateNewRating(
      { rating: p1Rating.rating, deviation: p1Rating.deviation, volatility: p1Rating.volatility },
      { rating: p2Rating.rating, deviation: p2Rating.deviation, volatility: p2Rating.volatility },
      p1Score
    );

    const newP2 = calculateNewRating(
      { rating: p2Rating.rating, deviation: p2Rating.deviation, volatility: p2Rating.volatility },
      { rating: p1Rating.rating, deviation: p1Rating.deviation, volatility: p1Rating.volatility },
      p2Score
    );
    ...
    await Promise.all([
      supabase.from('user_ratings').update({
        rating: newP1.rating,
