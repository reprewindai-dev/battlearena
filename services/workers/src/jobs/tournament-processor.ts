...
const log = createJobLogger('tournament-processor');

const generateRoomCode = (): string => {
  return Math.random().toString(36).substring(2, 8).toUpperCase();
};

export const progressTournaments = async () => {
...
const checkAndProgressBracket = async (tournamentId: string) => {
  ...
  for (const match of pendingMatches || []) {
    if (!match.battle_id) {
      ...
      if (battle) {
        await supabase.from('tournament_brackets').update({
          battle_id: battle.id,
          status: 'scheduled',
        }).eq('id', match.id);
      }
  ...
  for (let round = 0; round < numRounds; round++) {
    ...
    for (let match = 0; match < matchesInRound; match++) {
      ...
      if (round === 0) {
        ...
        if (p1Index < shuffled.length) bracket.player1_id = shuffled[p1Index].user_id;
        if (p2Index < shuffled.length) bracket.player2_id = shuffled[p2Index].user_id;
      }

      await supabase.from('tournament_brackets').insert(bracket);
    }
  }
};
