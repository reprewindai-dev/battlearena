...
exports.runEngagementAnalysis = exports.identifyPowerUsers = exports.calculateRetentionMetrics = exports.analyzeUserEngagement = void 0;
const supabase_1 = require("../lib/supabase");
const redis_1 = require("../lib/redis");
const logger_1 = require("../lib/logger");
const queue_1 = require("../lib/queue");
const log = (0, logger_1.createJobLogger)('user-engagement');
const analyzeUserEngagement = async () => {
    ...
    for (const user of inactiveUsers || []) {
        ...
        const { data: rating } = await supabase_1.supabase
            .from('user_ratings')
    ...
    for (const user of atRiskUsers) {
        await queue_1.notificationQueue.add('re-engagement', {
            userId: user.userId,
            type: 'at_risk',
    ...
    await redis_1.redis.hset('metrics:engagement', {
...
exports.analyzeUserEngagement = analyzeUserEngagement;
const calculateRetentionMetrics = async () => {
    ...
    for (const days of periods) {
        ...
        const { count: activeUsers } = await supabase_1.supabase
            .from('users')
            .select('id', { count: 'exact' })
            .gte('created_at', startDate)
            .lt('created_at', endDate)
...
const identifyPowerUsers = async () => {
    ...
    for (const user of activeUsers || []) {
        const { count: battleCount } = await supabase_1.supabase
            .from('battles')
            .select('id', { count: 'exact' })
            .or(`player1_id.eq.${user.id},player2_id.eq.${user.id}`)
            .gte('created_at', thirtyDaysAgo);
        const { data: spending } = await supabase_1.supabase
    ...
    await redis_1.redis.del('users:power');
    if (powerUsers.length > 0) {
        await redis_1.redis.sadd('users:power', ...powerUsers);
    }
    log.info({ count: powerUsers.length }, 'Power users identified');
    return powerUsers;
};
exports.identifyPowerUsers = identifyPowerUsers;
const runEngagementAnalysis = async () => {
    await (0, exports.analyzeUserEngagement)();
    await (0, exports.calculateRetentionMetrics)();
    await (0, exports.identifyPowerUsers)();
};
exports.runEngagementAnalysis = runEngagementAnalysis;
