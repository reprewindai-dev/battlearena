...
exports.processGrantApplications = exports.distributeTournamentPrizes = exports.calculateMonthlyCRF = void 0;
const supabase_1 = require("../lib/supabase");
const logger_1 = require("../lib/logger");
const log = (0, logger_1.createJobLogger)('crf-allocation');
...
const calculateMonthlyCRF = async () => {
    ...
    const { data: monthlyRevenue } = await supabase_1.supabase
    ...
    await supabase_1.supabase.from('crf_allocations').insert({
...
exports.calculateMonthlyCRF = calculateMonthlyCRF;
const distributeTournamentPrizes = async () => {
    log.info('Distributing tournament prizes from CRF');
    const { data: pendingPrizes } = await supabase_1.supabase
    ...
    for (const prize of pendingPrizes || []) {
        await supabase_1.supabase.from('wallets').update({
            points_balance: supabase_1.supabase.rpc('increment_points', { amount: prize.amount_cents }),
        }).eq('user_id', prize.user_id);
        await supabase_1.supabase.from('tournament_prizes').update({
            status: 'distributed',
            distributed_at: new Date().toISOString(),
        }).eq('id', prize.id);
        await supabase_1.supabase.from('token_transactions').insert({
...
const processGrantApplications = async () => {
    log.info('Processing grant applications');
    const { data: approvedGrants } = await supabase_1.supabase
    ...
    for (const grant of approvedGrants || []) {
        await supabase_1.supabase.from('wallets').update({
            points_balance: supabase_1.supabase.rpc('increment_points', { amount: grant.amount_cents }),
        }).eq('user_id', grant.user_id);
        await supabase_1.supabase.from('grant_applications').update({
            disbursed: true,
...
exports.processGrantApplications = processGrantApplications;
//# sourceMappingURL=crf-allocation.js.map